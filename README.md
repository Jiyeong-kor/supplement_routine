...
| iOS native | SwiftUI shell이 `SupplementRoutineShared` framework를 import/call하고 UserDefaults local snapshot을 저장/복원합니다. notification adapter는 후속 작업입니다. |
...
- iOS KMP 앱은 SwiftUI shell, shared module import/call smoke path, UserDefaults local snapshot adapter가 연결되어 있습니다.
- 남은 핵심 gap은 iOS notification adapter, Android/iOS QA, Android notification 실기기 QA, Flutter cutover 결정입니다.
