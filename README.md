...
> 참고: 이 README는 현재 전환 상태를 설명하기 위한 최소 갱신본입니다. 최종 릴리즈 README는 QA, Flutter cutover 결정, 스토어 제출 기준이 끝난 뒤 다시 정리합니다.
...
| iOS native | SwiftUI shell이 `SupplementRoutineShared` framework를 import/call하고 UserDefaults local snapshot, UserNotifications adapter를 연결합니다. |
...
| Notification | Flutter local notifications, Android native notification/exact alarm adapter, iOS UserNotifications adapter |
...
- iOS KMP 앱은 SwiftUI shell, shared module import/call smoke path, UserDefaults local snapshot adapter, UserNotifications adapter가 연결되어 있습니다.
- 남은 핵심 gap은 Android/iOS QA, Android notification 실기기 QA, Flutter cutover 결정입니다.
