...
이 문서는 Android와 iOS를 릴리스 직전까지 끌어올리기 위한 현재 검증 상태와 남은 외부 환경 QA를 정리한다. Flutter 기준 구현은 KMP parity, iOS signing, store cutover 완료 전까지 유지한다.
...
| iOS release signing secrets | 미등록 | 2026-06-05 `gh secret list --repo Jiyeong-kor/supplement_routine` 결과 iOS release signing secrets 없음. iOS signed archive/IPA 생성 전 등록 필요. [#67](https://github.com/Jiyeong-kor/supplement_routine/issues/67)에서 추적 |
...
| Android KMP Release workflow | 통과 | `KMP Release` run `27004890398`, `platform=android`, Android signed release artifacts job 성공. iOS job은 의도적으로 skipped |
| Android signed release artifacts | 생성됨 | `kmp-android-release` artifact 다운로드 확인. `androidApp-release.apk` 13,686,326 bytes, `androidApp-release.aab` 13,252,381 bytes |
| Android signed APK verification | 통과 | `apksigner verify --verbose --print-certs` 결과 `Verifies`, v1/v2 signing true |
| Android signed AAB verification | 통과 | `jarsigner -verify -verbose -certs` 결과 `jar verified`. 자체 서명 upload certificate라 PKIX chain warning은 예상됨 |
...
| Flutter 기준 구현 | KMP parity, iOS signing, store cutover 완료 전까지 유지 |
...
| Flutter cutover/removal | 현재 릴리스 직전 단계에서는 제거하지 않음. KMP parity, iOS signing, store cutover 완료 전까지 기준 구현과 rollback reference로 유지 |
...
- iOS distribution certificate/provisioning profile과 GitHub Actions iOS release signing secret 구성
- KMP manual release workflow iOS 실행 결과. Android는 `platform=android` run `27004890398`로 signed APK/AAB artifact 생성까지 확인됨
- Play Console/App Store Connect 등록과 스토어 제출 전 계정 설정
